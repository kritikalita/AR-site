...
const TargetInput = ({ as = "input", className, ...props }) => {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <motion.div
        animate={
          isFocused ? { opacity: 1, x: 0, y: 0 } : { opacity: 0, x: 10, y: -10 }
        }
...
const Footer = () => {
  ...
  return (
    <footer
      id="contact"
      className="bg-transparent text-white pt-16 md:pt-20 pb-10 border-t border-gray-800"
    >
        ...
        >
          <div>
            ...
            <div className="space-y-6 text-gray-300 text-sm md:text-base">
              <div className="flex items-start space-x-3">
                ...
                {/* 🔗 UPDATED: Made Address Clickable with the 'address link' */}
                <a
              ...
              <div className="w-full h-48 md:h-56 rounded-lg overflow-hidden border border-gray-800 relative group bg-gray-900">
                {/* 1. DECORATIVE OVERLAY (Visual only) */}
                <div className="absolute inset-0 bg-cyan-500/10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20"></div>

                {/* 2. CLICKABLE LINK OVERLAY */}
                <a
                  href="https://maps.app.goo.gl/YuVn4j5rvq8HKyVR7"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="absolute inset-0 z-30" // Highest Z-index to capture clicks
                  aria-label="Open in Google Maps"
                ></a>

                {/* 3. THE INTERACTIVE MAP (Iframe) */}
                <iframe
                  ...
                  className="relative z-10 w-full h-full" // Sits on top of the image
                ></iframe>

                {/* 4. FALLBACK IMAGE (New!) */}
                {/* This sits at z-0 (the back). If the iframe (z-10) fails to load or is transparent, this shows. */}
                <img
                  src="public/images/products/Map Fallback.png" // Replace with your own map screenshot if you have one
                  alt="Map Location"
          ...
          <div className="mt-auto pt-8 border-t border-gray-800 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500">
            <p className="text-center md:text-left">
              © 2023 Arrobot. All rights reserved.
            </p>
            <div className="flex space-x-4 mt-4 md:mt-0">
              ...
              <a
                href="https://www.youtube.com/@Raghu_Vamsi_Group"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-cyan-500 transition-colors"
              >
